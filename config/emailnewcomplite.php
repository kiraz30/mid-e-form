...
$mail->addReplyTo('', '');
$exe=mysqli_query($con,"SELECT c.Name Nama,c.Email
	...
	WHERE a.No_Req = '$id'   AND a.Status_Tracking = '1'
	GROUP BY c.Name,c.Email ORDER BY a.IDNo Asc  "); 
while(@$datakirim =mysqli_fetch_array($exe)){
	//$mail->AddAddress("{$datakirim['Email']}","{$datakirim['Approve']}");  //tujuan email
	...
	$mail->Body = "
	Dear All,<br><br>
	Request ".$WorkFlowMenu." is Complete <br>
	Request No : ".$id."<br>
