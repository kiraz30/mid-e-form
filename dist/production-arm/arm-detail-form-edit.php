<?php 
	...
	b.Resource_Code,b.SAP_Code,b.Remark FROM  tb_prod_add_resource a 
	INNER JOIN tb_prod_add_resource_detail b
...
<table width="100%" border="0" class="table table-striped" id="table table-striped" >
    <tr>
		...
		<select class="form-control" id="inputVendorCode" name="inputVendorCode" 
			...
			if ($indexNo=="1" || $indexNo=="4" & $indexProcess=="Step 1")
		...
		onClick="popupwindow('production-arm/vendor-master-popup.php?id=arm','Search Project Name','600','900');" 
		<?php ARMPurDisable($Approve_No,$indexProcess,'Step 1',$Divisi);  
			if ($indexNo=="1" || $indexNo=="4" & $indexProcess=="Step 1")
			{ echo" required";}else {echo $disabled;}	?> >
    ...
    <tr>
        ...
        <td ><span class="form-group">
			...
			name="inputVendorName" id="inputVendorName"  maxlength="100" type="text" placeholder="Input Vendor Name"    
		  	value="<?php echo @$tampildata['Vendor_Name']; ?>" readonly="readonly"/>
            </span> 
		</td>
    </tr>
	<tr>
		...
		<td ><span class="form-group">
			<input class="form-control py-4"
			<?php ARMPurDisable($Approve_No,$indexProcess,'Step 1',$Divisi);  
			if ($indexNo=="1" & $indexProcess=="Step 1"){ echo" required";}	?>
			name="inputPOLeadTime" id="inputPOLeadTime"  
			maxlength="50" type="text" placeholder="Input PO Lead Time"   
			style='text-align:right;' 
		  	value="<?php echo @$tampildata['PO_Lead_Time'];?>"  <?php echo $disabled; ?>/>
            </span> 
		</td>
    </tr>
	<tr>
		...
		<td ><span class="form-group">
			...
			style='text-align:right;' 
		  	value="<?php echo @$tampildata['Safety_Stock_Ratio'];?>" <?php echo $disabled; ?>/>
            </span> 
		</td>
    </tr>
	<tr>
        ...
        <td><span class="form-group">
			...
			id="selectOrderUM" name="selectOrderUM" <?php echo $disabled; ?>  >
		...
		<td ><span class="form-group">
			...
			style='text-align:right;' 
		  	value="<?php echo @$tampildata['Cate_of_Day']; ?>" <?php echo $disabled; ?> />
            </span> 
		</td>
    </tr>
	<tr>
        ...
        <td  ><span class="form-group">
			...
			style='text-align:right;' 
			value="<?php echo @$tampildata['Minimum_Order_Qty']; ?>" <?php echo $disabled; ?>    />
            </span>
		</td>
		...
		<td ><span class="form-group">
			...
			maxlength="50" type="text" placeholder="Input Buyer Planner"  
		  	value="<?php  echo @$tampildata['Buyer_Planner'];?>" <?php echo $disabled; ?> />
            </span> 
		</td>
    </tr>
	<tr>
        ...
        <td  ><span class="form-group">
    	    <input class="form-control py-4"  
			name="inputMultipleOrderQty" id="inputMultipleOrderQty" 
			maxlength="150" type="text" placeholder="Input Multiple Order Qty" onKeyUp="return angka(this);" 
			style='text-align:right;' 
			value="<?php echo @$tampildata['Miltiple_Order_Qty']; ?>"  <?php echo $disabled; ?>  />
            </span>
		</td>
		...
		<td ><div class="form-group d-flex align-items-center justify-content-between mt-0 mb-1">
			...
			maxlength="50" type="text" placeholder="Input Resource Code"  
		  	value="<?php echo @$tampildata['Resource_Code'];  ?>" 
			...
			?>
			class="btn btn-primary" title="Search Project Name"  name="btnsearch" 
			onClick="popupwindow('production-arm/pm-popup.php?id=ps','Search production Material ','600','900');">
			<span class="glyphicon glyphicon-search" ></span> </button> 
			</div> 
		</td>	
	...
	<tr>
        <td><span class="form-group">Currency *</span></td>
        <td><span class="form-group">
			<select style="padding:2px 2px 2px 2px" name="selectCurency"
			 
			id="selectCurency" class="form-control" <?php echo $disabled; ?>   >
	...
	<tr>
        ...
        <td><span class="form-group">
			...
			style='text-align:right;'  value="<?php echo @$tampildata['Std_UP']; ?>" <?php echo $disabled; ?>    />
            </span>
		</td>
...
function get_save_form(){
  ...
  var inputRemark			= $('#inputRemark').val();
   $.ajax({
   type: 'POST',
   url: "production-arm/arm-detail-form-save-new.php",
