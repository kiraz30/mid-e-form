<?php 
	...
	include "../../config/connect_sql.php";
 	$query="SELECT a.Request_No,a.MPR_Code,a.Project_Name,
	...
	INNER JOIN tb_prod_add_resource_detail_w_p b
...
<table width="100%" border="0" class="table table-striped" id="table table-striped" >
	...
	<tr>
        ...
        <td colspan="2">
			<div class="form-group d-flex align-items-center justify-content-between mt-0 mb-1"> 
				<input class="form-control py-4"  
				name="inputResourceCode" id="inputResourceCode" maxlength="150" type="text" placeholder="Input Material Code"   
				...
				name="btnsearch"
				onClick="popupwindow('production-arm/pm-popup.php?id=armf2','Search Material','600','900');"
				<?php echo $disabledProd; ?>><span class="glyphicon glyphicon-search" ></span></button>
	...
	<tr>
		...
		<button type="button" id="edit_form" name="edit_form" value="Save" 
		 class="btn btn-primary" onclick="myFunctionProd()" <?php echo $disabledProd; ?> >Save</button>
	</td>
    </tr>
</table>
</form>
...
<script type="text/javascript">
 ...
 function myFunction() {
	...
	if(document.getElementById('inputFinishGoodCode').value==""){
		...
		if (confirm(text) == true) {
			get_save_form();
			document.getElementById("myCloseWP").click();
			tampil_data_arm();
		} 
	}	
  ...
  var inputRemark			= $('#inputRemark').val();
   $.ajax({
   type: 'POST',
   url: "production-arm/arm-detail-form-save-new.php?type=work-process",
