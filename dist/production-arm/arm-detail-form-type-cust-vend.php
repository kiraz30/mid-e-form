...
<script language="JavaScript">
	function setFocus(){
		document.faw.inputProjectName.focus();	
	}
</script>
...
<link href="../vendor/font-awesome/css/font-awesome.min.css" rel="stylesheet" type="text/css">
    ...
    <div class="container-fluid"> 
      <h3 class="mt-4"><?php if ($button=="prod-arm-add" || $button=="prod-arm-revise") 
		...
		<li class="breadcrumb-item"><a href="../dist/index.php?button=prod-arm-edit&id=<?php echo $_GET['id']; ?>">New Request Additional Resource Master</a></li>
        <li class="breadcrumb-item active"><?php if ($button=="prod-arm-add" || $button=="add-revise-prod-arm") 
 	...
 	<div class="card mb-4">
		...
		$exeNext = mysqli_query($con,"SELECT a.KDDivision,a.KDPosition
			...
			WHERE c.Request_No='ARMI-2024-09-0003' And a.KDDivision='F01' 
			AND b.TRACKING_CODE='2' GROUP BY  a.KDDivision,a.KDPosition");
			while(@$row =mysqli_fetch_array($exeNext)){
				$appdivisi= @$row['KDDivision'];
				$appposisi= @$row['KDPosition'];
				$appdivisi = explode(",", $appdivisi);
				$appposisi = explode(",", $appposisi);
				foreach($appdivisi as $appdivisi):
					echo $appdivisi."','";
						
				 
				endforeach;
			
				foreach($appposisi as $appposisi):
					echo $appposisi."','";
				endforeach;
		...
		a.Project_Name,a.Subject,a.`Type`,a.Content,a.Type_Vendor,a.Type_Customer,
		...
		?>
          <table width="100%" border="0" class="table table-striped" id="dataTables-example" >
            ...
            <tr>
			  ...
			  value="<?php if (@$tampildata['LAST_TRACK']=="Complete" & $button=="arm-revise") 
				...
				<table cellpadding="0" cellspacing="0" border="0"width="100%"  >
					<tr>
						<td style="text-align:left; width:150px; height:20px;">
							<label><input type="checkbox" name="ChkType5" id="ChkType5" onclick="return false;"
							<?php if (@$tampildata['Type_Vendor']=="1") { echo 'checked="checked"';} ?>  value="<?php echo @$tampildata['Type_Vendor']  ?>"> Vendor</label></td>
						<td style="text-align:left; width:150px; height:20px;">
							<label><input type="checkbox" name="ChkType6" id="ChkType6" onclick="return false;"
							<?php if (@$tampildata['Type_Customer']=="1") { echo 'checked="checked"';} ?>  value="<?php echo @$tampildata['Type_Customer']  ?>" > Customer</label></td>
					</tr>
				</table>
			  </td>
			</tr>
		  ...
		  <?php if (@$tampildata['LAST_TRACK']!="7" || (@$tampildata['LAST_TRACK']=="7"  & $button=="arm-revise") )  
		  {echo $disabled;} else {echo "disabled='disabled'";} ?>>Sent To Approval</button>
		  
		  
		  <?php if (@$tampildata['LAST_TRACK']=="7" & $button!="arm-revise")  {?>
		  <button type="submit" name="Save" value="Revise" onClick="return checkRevise()" 
		  class="btn btn-primary">Revise</button>
	      <?php ;} ?>
		  <?php if (@$tampildata['LAST_TRACK']=="7" & $button!="arm-revise" || @$tampildata['LAST_TRACK']=="Revise" & $button!="add-revise-faw")  {?>
...
<script type="text/javascript">
   $(document).ready(function(){
	   $('#add').on('show.bs.modal', function (e) {
		   ...
		   var ChkVendor 		= $('#ChkType5').val(); 
		   var ChkCustomer 		= $('#ChkType6').val(); 
		   $.ajax({
			   type :'post',
			   url: "production-arm/arm-detail-form-type-cust-vend-add.php",
			   data: { getDetail: getDetail, AutoRequestNo: AutoRequestNo,InputSubject:InputSubject,
				ChkVendor: ChkVendor,ChkCustomer:ChkCustomer},
...
$(document).ready(function(){
 ...
 function get_ProductionARMDetail(){
  ...
  $.ajax({
   type: 'POST',
   url: "production-arm/arm-detail-form-type-cust-vend-view.php",
   data: {data2: b},
