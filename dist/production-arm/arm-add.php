...
<html lang="en">
    ...
    <div class="container-fluid"> 
      <h3 class="mt-4"><?php if ($button=="prod-arm-add" || $button=="add-revise-arm") 
		{echo "New Request Additional Resource Master";} else  {echo "Edit Request Additional Resource Master";} ?> </h3>
	    <ol class="breadcrumb mb-4">
        <li class="breadcrumb-item"><a href="../dist/index.php?button=dashboard">Dashboard</a></li>
		<li class="breadcrumb-item"><a href="../dist/index.php?button=prod-arm">Additional Resource Master</a></li>
        <li class="breadcrumb-item active"><?php if ($button=="prod-arm-add" || $button=="add-revise-arm") 
 	...
 	<div class="card mb-4">
		...
		if (@$tampildata['LAST_TRACK']<>"1" &  @$tampildata['LAST_TRACK']<>"3" & @$tampildata['LAST_TRACK']<>"" & 
			$button<>"add-revise-arm" & $button<>"revise-arm")
		{$disabled="disabled";
	...
	if($_POST){
		...
		if($Save=="Save"){
			...
			WHERE UserDomain= '$username' And WorkFlowMenu= 'ARM' And LevelApproval ='Requestor'");
			if (mysqli_num_rows($TanyaReqWokflow) ==0 ) {  
				...
				if (mysqli_num_rows($Tanya) !=0 ) {  
					...
					if ($button=="prod-arm-add") {
						...
						SaveTracking($NomorReq,'1',"",$username,$createddate,$hostname);
					}else {
						include "arm-save-new.php";
		...
		elseif ($Save=="Revise"){
			...
			SaveTracking($NomorReq,'3',"Request pengajuan direvisi",$username,$createddate,$hostname);
			//_________________________________________________________________________________________
			$message = "Data successfully Revise";
			echo "<script type='text/javascript'>alert('$message');</script>";
			echo"<script>  window.location='../dist/index.php?button=revise-arm&id=$inputAutoRequestNo'; </script>";
		...
		elseif($Save=="Cancel"){ 
			...
			SaveTracking($NomorReq,'5',"Request pengajuan dibatalkan",$username,$createddate,$hostname);
		...
		elseif($Send=="NextandSave"){ 
			...
			if (@$valType2==1 || $valType4==1 )  {
				echo"<script>  window.location='../dist/index.php?button=prod-arm-detail-add&id=$inputAutoRequestNo'; </script>";
			}else if (@$valType3==1)  {
				echo"<script>  window.location='../dist/index.php?button=prod-arm-detail-type-work-process&id=$inputAutoRequestNo'; </script>";
			}else if (@$valType5==1 || $valType6==1 )  {
				echo"<script>  window.location='../dist/index.php?button=prod-arm-detail-type-cust-vend&id=$inputAutoRequestNo'; </script>";
			}else{
				echo"<script>  window.location='../dist/index.php?button=prod-arm-detail-type-finish-goods&id=$inputAutoRequestNo'; </script>";
			}
		}elseif($Send=="Next"){ 
			if (@$valType2==1 || $valType4==1 )  {
				echo"<script>  window.location='../dist/index.php?button=prod-arm-detail-add&id=$inputAutoRequestNo'; </script>";
		...
		?>
          <table width="100%" border="0" class="table table-striped" id="dataTables-example" >
            <tr>
			  ...
			  value="<?php if ($button=="prod-arm-add") {echo $NomorReq;} elseif ($button=="add-revise-arm") 
            ...
            <tr>
			  ...
			  value="<?php if (@$tampildata['LAST_TRACK']=="Complete" & $button=="add-revise-arm") 
			...
			<tr>
			  ...
			  <select class="form-control" id="InputType" name="InputType" <?php echo $disabled; ?>>
					...
					<option value="Finish Goods" <?php if (@$tampildata['Type']=='Finish Goods') {echo "Selected"; }?> >Finish Goods</option>
			...
			<tr>
              ...
              <td colspan="2">
				<table cellpadding="0" cellspacing="0" border="0"width="100%"  >
					<tr>
						<td style="text-align:left; width:120px; height:20px;">
							<label><input type="checkbox" name="ChkSegment1" id="ChkSegment1" <?php echo $onclick; ?>
							<?php if (@$tampildata['Segment_Price']=="1") { echo 'checked="checked"';} ?>
							onclick="return false;"> Price</label></td>
						<td style="text-align:left; width:120px; height:20px;">
							<label><input type="checkbox" name="ChkSegment2" id="ChkSegment2" <?php echo $onclick; ?>
							<?php if (@$tampildata['Segment_Data_Informasi']=="1") { echo 'checked="checked"';} ?> > Data Informasi</label></td>
						<td style="text-align:left; width:120px; height:20px;">
							<label><input type="checkbox" name="ChkSegment3" id="ChkSegment3" <?php echo $onclick; ?>
							<?php if (@$tampildata['Segment_Ukuran']=="1") { echo 'checked="checked"';} ?>  > Ukuran</label></td>
						<td style="text-align:left; width:120px; height:20px;">
							<label><input type="checkbox" name="ChkSegment4" id="ChkSegment4" <?php echo $onclick; ?>
							<?php if (@$tampildata['Segment_Others']=="1") { echo 'checked="checked"';} ?> > Others</label></td>
						<td style="text-align:left; width:150px; height:20px;">
							<label><input type="checkbox" name="ChkSegment5" id="ChkSegment5" <?php echo $onclick; ?>
		  ...
		  <?php } else { ?>
			<button type="submit" name="Send" value="Next"
		  	...
		  	<span   class="glyphicon glyphicon-menu-right" title="Save and Continue "></span></button>
		  <?php } ?>
		  <?php if (@$tampildata['LAST_TRACK']<>"6")  {?>
		  <button type="submit" name="Save" 
		  value="<?php if ($button=="prod-arm-add" || $button=="arm-revise"|| $button=="add-revise-prod-arm") 
		  {echo"Save";} else {echo"Update";}  ?>"
		  <?php if ($button=="prod-arm-add" || $button=="arm-revise" || $button=="add-revise-prod-arm") 
		   {echo 'onclick="return checkDraft(arm)"';} else {echo 'onclick="return checkEdit(arm)"';} ?>  
		  class="btn btn-primary"
		  <?php if (@$tampildata['LAST_TRACK']!="7" || (@$tampildata['LAST_TRACK']=="8"  & $button=="add-revise-prod-arm") )  
		  {echo $disabled;} else {echo "disabled='disabled'";} ?>>Draft</button>
		  <?php ;} ?>
		  
		  <?php if (@$tampildata['LAST_TRACK']=="8" & $button!="add-revise-prod-arm")  {?>
		  <button type="submit" name="Save" value="Revise" onClick="return checkRevise()" 
		  class="btn btn-primary">Revise</button>
	      <?php ;} ?>
		  <?php if (@$tampildata['LAST_TRACK']=="8" & $button!="add-revise-prod-arm" || @$tampildata['LAST_TRACK']=="3" & $button!="add-revise-prod-arm")  {?>
...
$(document).ready(function(){
 ...
 //___________________________________________________________
	...
	if (form.inputAutoRequestNo.value == ""){
    	...
    	return (false);  		}
	else if(form.ChkType1.checked ==false & form.ChkType2.checked ==false& form.ChkType3.checked ==false &
	form.ChkType4.checked ==false & form.ChkType5.checked ==false & form.ChkType6.checked ==false){
		alert("Type Can not be empty!");
    	return (false);  		}
	else if(form.ChkSegment1.checked ==false & form.ChkSegment2.checked ==false & form.ChkSegment3.checked ==false &
	form.ChkSegment4.checked ==false & form.ChkSegment5.checked ==false){
