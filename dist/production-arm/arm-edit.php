<?php 
	mysqli_query($con,"UPDATE tb_prod_add_resource SET 
	Project_Name='$inputProjectName',Subject='$InputSubject',
		...
		for($i=0;$i<count(@$_FILES['InputFileDocLampiran']);$i++){
			...
			if (mysqli_num_rows($exe) ==0 ) {  
				if(trim($name)!=''){
					...
					mysqli_query($con,"UPDATE  tb_prod_add_resource_doc_lampiran SET DocLampiran='$InputName' 
					where No_ID = '$temp' ");
			}
		}
	}
