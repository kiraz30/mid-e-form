...
<html lang="en">
	...
	function changeparent(doc1,doc2,doc3,doc4){
        var input1=doc1;
        var input2=doc2;
		var input3=doc3;
		var input4=doc4;
		window.opener.document.getElementById('inputProjectName').value=input1;
		window.opener.document.getElementById('InputNprfCode').value=input2;
		window.opener.document.getElementById('SelectTypeRequest').value=input3;
		window.opener.document.getElementById('inputThemaNumber').value=input4;
	...
	<main> 
    <div class="container-fluid"><h3 class="mt-4">Search NPRF</h3>
...
<?php
	 ...
	 {  ?>
		  <table width="100%" class="table table-striped table-bordered table-hover" id=" " >
			...
			$query="SELECT ID_No,Request_No,Thema_Number,Thema_Name,Type_Request,
			CreatedBy,date(CreatedDate) as CreatedDate,Mcj,Status_NPRF FROM tb_nprf 
			Where Status_NPRF='Complete By MCJ'";
			if(@$Search=="")	{
				$query =$query; }
			else{
				$query .= " And (Request_No like '%$Search%' or Thema_Number like '%$Search%' or 
				Thema_Name like '%$Search%' or Type_Request like '%$Search%')";
			}
			...
			?>
			  ...
			  <tr> 
				...
				onClick="javascript:changeparent('<?php echo $row['Thema_Name']; ?>','<?php echo $row['Request_No']; ?>','<?php echo $row['Type_Request'];?>','<?php echo $row['Thema_Number'];?>');">
