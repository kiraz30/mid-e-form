...
<html lang="en">
    ...
    <div class="container-fluid"> 
 	...
 	<div class="card mb-4">
        ...
        <?php
		$exe = sqlsrv_query($myConn,"SELECT Item_Code,Item_Name,DZ_per_CT,Netto,UOM,Barcode,
		Discontinue,DiscontinueDate,convert(datetime, LaunchingDate, 103) as LaunchingDate 
		from Master_Item Where Discontinue=0 And Item_Code = '".@$_GET['id']."'");
        $tampildata=sqlsrv_fetch_array($exe);
		
	 
		
		$exeFlex = sqlsrv_query($myConnFlex,"SELECT TOP 1 a.aoldate, a.ParentObjectId, b.ResourceUK,a.aotDisctn from AOSTOCKSTATUS a 
		INNER JOIN fdBasResc b ON a.ParentObjectID=b.ObjectID Where b.ResourceUK = '".@$_GET['id']."'");
        $tampildataFlex=sqlsrv_fetch_array($exeFlex);
		...
		else if($inputLaunchingDate==""){  
			echo "<div class='form-group has-error'>
			<label class='control-label' for='inputError'>Launching Date Can not be empty *</label></div>"; }
		elseif($button=="edit-resource"){ 
			if($tampildataFlex['aotDisctn']<>""){
				$discountinue=date_format($tampildataFlex['aotDisctn'],"Y/m/d");
			}else {$discountinue="0000-00-00";}
			...
			if ($valKoreksiLaunchingDate==1) {
				...
				if (mysqli_num_rows($Tanya) ==0 ) {
					...
					'".$tampildata['Barcode']."','".$discountinue."',
			...
			sqlsrv_query($myConn,"update Master_Item set LaunchingDate='$inputLaunchingDate' WHERE Item_Code='".@$_GET['id']."'");
			sqlsrv_query($myConnFlex,"update AOSTOCKSTATUS set aoldate='$inputLaunchingDate' 
			WHERE ParentObjectId='".$tampildataFlex['ParentObjectId']."'");
			
			
			LogInfo($createddate,"Save : Form Edit Master Item Product Code [".$_GET['id']."]
			Old Launching Date [".$discountinue."] New Launching Date [".$inputLaunchingDate."]",$username,$hostname);
		...
		<table width="100%"  class="table table-striped  table-hover" id="dataTables-example">
		  ...
		  <tr>
			...
			<td width="30%"><?php if (@$tampildata['LaunchingDate']!="")   {echo date_format(@$tampildata['LaunchingDate'],"d/m/Y");} ?></td>
			<td width="20%"></td>
		  </tr>
		  <tr>
			<td width="20%"><span class="form-group">Display K2 Sales Order</span> </td>
			<td align="right"><span class="form-group">
				<input class="form-control py-1" name="inputLaunchingDate" id="inputLaunchingDate"  maxlength="20" type="date" min="2010-01-01"
				value="<?php echo date_format(@$tampildata['LaunchingDate'],"d/m/Y"); ?>" title="Date Format DD/MM/YYYY" data-date-format="DD MMMM YYYY"  /> 
				</span>
			</td>
			<td width="30%"><input class="form-check-label" name="ChkKoreksiLaunchingDate" id="ChkKoreksiLaunchingDate" type="checkbox" 
				value="<?php echo date_format($tampildata['LaunchingDate'],"d/m/Y"); ?>" title="Checked for Original Launching Date Corection" 
				<?php $Tanya = mysqli_query($con,"SELECT * FROM tb_master_item_flexprocess WHERE Item_Code = '".@$_GET['id']."'");
				if (mysqli_num_rows($Tanya) <>0 ) { 
					echo ''; 
				} else { 
					echo 'checked="checked" readonly="readonly"';} ?>/> Original Launching Date Corection </td>
			<td width="20%"></td>
		  </tr>
		</table>
