<form name="specrevisi" id="specrevisi" action="" method="post"  >
<?php
	$Send						= @$_POST['Send'];
	$datenow					= date("Y-m-d");
	...
	a.DznCtn,a.DznCtn_Keterangan,a.WeighOfContenCtn,Status_Spec,a.Remark,a.CreatedBy,a.CreatedDate
...
<table width="100%" border="0" class="table table-striped" id="dataTables-example" >
	...
	<tr>
		<td width="1%"><span class="form-group"></span>II.Product Development</td>
	...
	<tr>
		<td><span class="form-group"></span></td>
		<td><span class="form-group">Net</span></td>
		<td>
	...
	<tr>
		<td rowspan="2" ><span class="form-group"></span></td>
	...
	<tr>
		...
		<td></td>
			  ...
			  maxlength="100"><?php echo @$tampildata['Remark']; ?></textarea></div>
		</td>
	</tr>
	<tr>
        <td colspan="2">Workflow Revise *</td>
        <td colspan="3">
			<div class="form-group">
				<select class="form-control" id="SelectWorkflowRevise" name="SelectWorkflowRevise"  >
				<option value="-">Select Workflow Revise</option>
					<?php
			  			$exe = mysqli_query($con,"SELECT Step_Revise, c.DivisionName FROM tb_workflownprf a
						  INNER JOIN tb_user b ON a.NameApproval=b.UserDomain
						  INNER JOIN tb_division c ON b.KDDivision=c.KDDivision
						  WHERE Request_No='$id' AND Approve_No=1
						  Group BY c.DivisionName ORDER BY Step_Revise" );
						while(@$row =mysqli_fetch_array($exe)){
							 
							  
							if($SelectWorkflowRevise == $row['DivisionName']){
								$cek = 'Selected';
							}else{
								$cek = '';
							}
							
							echo"<option value='".$row['Step_Revise']."' $cek>".$row['DivisionName']."</option>";
	...
	<tr>
        ...
        <td colspan="3"><div class="form-group"> <textarea cols="4" id="inputWorkflowRemark"  name="inputWorkflowRemark"  
			class="form-control py-4" placeholder="Workflow Remark"></textarea></div>
		</td>
    </tr>
	<tr>
        <td colspan="4"> 
		  <button type="button" id="edit_form" name="edit_form" value="Save" 
		onclick="myFunction()" class="btn btn-primary">Revise</button>
	</td>
    </tr>
</table>
</form>
...
<script type="text/javascript">
 ...
 function get_Del_form(){
  $('#inputAutoRequestNo').val("");
  $('#inputWorkflowRemark').val("");
  $('#SelectWorkflowRevise').val("");

 }
 function myFunction() {
	if(document.getElementById('inputAutoRequestNo').value==""){
		alert("Request No Can not be empty *");
		document.getElementById('inputAutoRequestNo').focus();
        return false;  }
	else if(document.getElementById('SelectWorkflowRevise').value=="-"){
        alert("Workflow Revise Can not be empty *");
		document.getElementById('SelectWorkflowRevise').focus();
        return false;  }
	else if(document.getElementById('inputWorkflowRemark').value==""){
        alert("Workflow Remark Can not be empty *");
		document.getElementById('inputWorkflowRemark').focus();
        return false;  }
...
function get_save_form(){
  var inputAutoRequestNo			= $('#inputAutoRequestNo').val();
  var SelectWorkflowRevise 			= $('#SelectWorkflowRevise').val();
  var inputWorkflowRemark			= $('#inputWorkflowRemark').val();
 
   $.ajax({
   type: 'POST',
   url: "spec-product/spec-product-save-workflow-revise.php",
   
   data: { inputAutoRequestNo	: inputAutoRequestNo,
	SelectWorkflowRevise		: SelectWorkflowRevise,
	inputWorkflowRemark			: inputWorkflowRemark,


   },
   success: function(info) {
 		alert("Data successfully Revise To Approval");	
		window.location='../dist/index.php?button=spec-product';
