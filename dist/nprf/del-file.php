...
if ($idpalavras) {
	...
	$exe =mysqli_query($con,"SELECT ID_No,Request_No,File FROM tb_fnim_file where ID_No = '".$idpalavras."' ");
	while(@$rowFile=mysqli_fetch_array($exe)){	
		if (!empty($rowFile['File'])){
			unlink($file.@$rowFile['File']);
			mysqli_query($con,"Delete From tb_fnim_file WHERE ID_No = '".$idpalavras."' "); 
			$success = true;
		}
	}
}
