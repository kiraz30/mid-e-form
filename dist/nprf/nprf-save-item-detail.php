...
if (is_array(@$_POST['tempItemDetail'])){
	for($i=0;$i<count(@$_POST['tempItemDetail']);$i++){
		//if($_POST['tempItemDetail'][$i]<>"")	{
			...
			NamaCurrencyTargetCOGS,Target_COGS,COGS,Sales_3Mth,Introduction,
			Satuan_Sales1Yr,Sales_1yr,Index_No,CreatedBy,CreatedDate,CreatedHostName) 
			...
			'$selectMataUangTargetCOGS','$InputTargetCOGS','$InputCOGS','$InputSales3Mth','$InputIntroduction',
			'$selectSatuanSales1Yr','$InputSales1Yr','$i+1','$username','$createddate','$ip : $hostname')");
...
if (is_array(@$_POST['tempCompetitor'])){
	for($i=0;$i<count(@$_POST['tempCompetitor']);$i++){
		//if($_POST['tempCompetitor'][$i]<>"")	{
...
if (is_array(@$_POST['InputOutlineofSchedule'])){
	for($i=0;$i<count(@$_POST['InputOutlineofSchedule']);$i++){
		//if($_POST['tempOutlineofSchedule'][$i]<>"")	{
