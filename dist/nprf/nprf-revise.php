...
<script type="text/javascript">
		function addRow(tableID) {
			...
			InputHPJ.setAttribute('style',"padding:2px 2px 2px 2px");
			...
			C_FPrice.setAttribute('style',"padding:2px 2px 2px 2px");
			...
			formulasamplecode.setAttribute('title',"Input Target COGS Exemple 1.000,25");
			...
			InputSales1Yr.setAttribute('style',"padding:2px 2px 2px 2px");
...
<script type="text/javascript">
		function addRowCompetitor (tableID) {
			...
			var cell4 = row.insertCell(3);
			var array = [<?php
			$div = mysqli_query($con,"SELECT NamaCurrency FROM tb_currency Group By NamaCurrency ");
			while($b = mysqli_fetch_array($div)){
			echo  "\"$b[NamaCurrency]\",";}	?>];
			...
			InputPriceCompetitor.setAttribute('style',"padding:2px 2px 2px 2px");
...
<html lang="en">
    ...
    <div class="container-fluid"> 
      <h3 class="mt-4"><?php if ($button=="add-nprf") {echo "New Request NPRF";} else  {echo "Revise Request NPRF";} ?> </h3>
	    <ol class="breadcrumb mb-4">
        <li class="breadcrumb-item"><a href="../dist/index.php?button=dashboard">Dashboard</a></li>
		<li class="breadcrumb-item"><a href="../dist/index.php?button=nprf">New Product Review Form D/E (NPRF)</a></li>
        <li class="breadcrumb-item active"><?php if ($button=="add-nprf") {echo "New Request NPRF";} else  {echo "Revise Request NPRF";} ?> </li>
      </ol>
 	<div class="card mb-4">
		...
		DATE_FORMAT(Launching_Date, '%Y') Tahun,Objective_Aim,Goal_Indicator,
		Market_Situasion,T_Consumer_M,T_Consumer_F,T_Age_GroupStart,T_Age_GroupEnd,
		T_Sosial_Economic_Class,Status_NPRF,Proposed_Concept,Request_for_Content,Request_for_Design,Competitor,Distribution,
		...
		Others,Outline_of_Schedule,Remark,Mcj,Master_Schedule,RemarkafterComplete,CreatedBy
		...
		if ($tampildata['Status_NPRF']<>"Draft" && $tampildata['Status_NPRF']<>"" && $tampildata['Status_NPRF']<>"Revise") {$disabled="disabled";} else{$disabled="";}

		//________________________________________________________________________________WORKFLOWNPRF
		$exeRevise = mysqli_query($con,"SELECT Step_Revise,LevelProcess,Index_No,Revise FROM tb_workflownprf  WHERE Request_No = '".@$_GET['id']."'
	...
	elseif($Send=="Send"){ 
		...
		ON  a.WorkFlowProcess =b.UserDomain AND a.WorkFlowMenu=b.WorkFlowMenu AND a.WorkFlowMenu='NPRF' 
		...
		if (mysqli_num_rows($exeNext) !=0 ) { 
			...
			//Simpan Inbox
			mysqli_query($con,"UPDATE tb_inbox Set Thema_Name='$inputThemaName',Request_Type='$SelectTypeRequest',
	...
	//End CRUD----------------------------------------------------------------------
	$exe =mysqli_query($con,"SELECT * FROM tb_nprf where Request_No = '".@$_GET['id']."'
	And Status_NPRF<>'Revise'");
	if (mysqli_num_rows($exe) !=0 ) { 
	echo"<h3>Tidak ada request yang harus di Revise</h3><br><br>";
	echo'<a class="btn btn-primary" href="../dist/index.php?button=nprf" title="Back NPRF">Back</a>';}
	
	else {
		?>
          <table width="100%" border="0" class="table table-striped" id="dataTables-example" >
            <tr>
              <td width="264"><span class="form-group">Request No</span></td>
			  ...
			  placeholder="Auto Request No" readonly="readonly" value="<?php if ($button=="add-nprf") {echo $NomorReq;} else {echo $tampildata['Request_No'];} ?>" />
              </span> </td>
            </tr>
            <tr>
              <td>Thema Number</td>
              <td colspan="2"><span class="form-group">
				...
				value="<?php if ($_POST) { echo $inputThemaNumber; } else {echo $tampildata['Thema_Number'];} ?>" readonly="readonly" />
              </span></td>
            </tr>
            <tr>
			...
			<tr>
              <td>Sent To *
			  <?php if ($tampildata['Status_NPRF']=="Revise"){?>
            ...
            <tr>
              <td height="70">
			  	<label><input type="checkbox" name="Prioritypoint1" <?php echo $disabled;  if ($tampildata['Priority_Point_EmphasisOnPrice']==1) { echo 'checked="checked"';} ?>>
			...
			<tr>
              ...
              <td colspan="2"><textarea class="ckeditor"  id="inputObjectiveAim"  name="inputObjectiveAim" 
				placeholder="Enter Objective / Aim" <?php echo $disabled; ?> ><?php if ($_POST) { echo $inputObjectiveAim; }  else { echo $tampildata['Objective_Aim'];} ?></textarea></td>
            </tr>
			<tr>
              ...
              <td colspan="2"><textarea  class="ckeditor"  id="inputGoalIndicator"  name="inputGoalIndicator"  
				...
				<?php echo $disabled; ?> ><?php if ($_POST) { echo $inputGoalIndicator; }   else { echo $tampildata['Goal_Indicator'];} ?></textarea></td>
            </tr>
			<tr>
              ...
              <td colspan="2"><span class="form-group">
				...
				placeholder="Enter Market Situasion" <?php echo $disabled; ?>><?php if ($_POST) { echo $inputMarketSituation; }  else { echo $tampildata['Market_Situasion'];} ?></textarea>
              </span></td>
            </tr>
            <tr>
              <td rowspan="4" >Target *
...
<button type="button" style="padding:0px 0px 0px 0px" class="btn btn-primary" title="Add Competitor"  
				  ...
				  <tr valign="bottom" align="center" bgcolor="#999999">
					...
					<th width="20%">Reason (s) Why a competitive product sells well</th>
					 ...
					 <?php
					$exe = mysqli_query($con,"SELECT ID_No,New_Product,Isi_Net,Netto,NamaCurrency,
					Price,Reason,Index_No 
				  ...
				  <tr>
					...
					<td>
						...
						id="selectMataUangCompetitor[]" <?php echo $disabled; ?> class="form-control">
						 <?php
							$div = mysqli_query($con,"SELECT NamaCurrency FROM tb_currency");
							while($b = mysqli_fetch_array($div)){
						...
						value="<?php echo number_format($rowNPRFCompetitor['Price'], 2, ",", "."); ?>" <?php echo $disabled; ?>></div>
					</td>
					<td>
					<textarea cols="4" id="InputReasonCompetitor[]"  name="InputReasonCompetitor[]"  
			  class="form-control py-4" placeholder="Enter Product Competitor" 
				<?php echo $disabled; ?>><?php echo $rowNPRFCompetitor['Reason']; ?></textarea></td>
				  </tr>
				  <?php $no++;} ?>
			  </table></td>
            </tr>
            <tr>
			  ...
			  <span class="form-group"> 
				...
				elseif (!empty($tampildata['Proposed_Concept']) & $tampildata['Status_NPRF'] =="Revise"){caridata2('tb_templete','Isi_Templete','ID_No','4','Status','1');} else { echo $tampildata['Proposed_Concept'];}  ?></textarea>
        		</span>			   
				</td>
            </tr>
            <tr>
              ...
              <td colspan="2"><span class="form-group">
				...
				elseif (!empty($tampildata['Request_for_Content']) & $tampildata['Status_NPRF'] =="Revise"){caridata2('tb_templete','Isi_Templete','ID_No','5','Status','1');} else { echo $tampildata['Request_for_Content'];}  ?></textarea>
              </span></td>
            </tr>
            <tr>
              ...
              <td colspan="2">
				...
				placeholder="Enter Proposed Concept" <?php echo $disabled; ?>><?php if ($_POST) { echo $inputRequest_for_Design; }   else { echo $tampildata['Request_for_Design'];} ?></textarea>
              </span></td>
            </tr>
            <tr>
...
<button type="button" style="padding:0px 0px 0px 0px" class="btn btn-primary" title="Add Item Detail"  
				  ...
				  <?php
					$exe = mysqli_query($con,"SELECT ID_No,MCJ_Item_No,New_Product,Status_Product,Isi_Net,Netto,NamaCurrency,
		  ...
		  {echo "readonly='readonly'";} ?>></td>
					<td style="padding:8px 2px 2px 2px"><textarea cols="4" id="InputNewProduct[]"  name="InputNewProduct[]"  
			  class="form-control py-4" placeholder="Enter Product Competitor" 
			    ...
			    <td colspan="3">
				 <table name="nprfCompetitor" id="nprfOutlineofSchedule"  width="100%" border="1" class="table table-striped table-bordered table-hover" >
				  <tr>
					...
					<span class="glyphicon glyphicon-trash" title="Delete Outline of Schedule" ></span></button>
					</td>
				  </tr>
				  <tr valign="bottom" align="center" bgcolor="#999999">
					<th width="1%"></th>
            ...
            <tr>
              ...
              <td colspan="2"><div class="form-group"> <textarea cols="4"  name="inputRemark"  class="form-control py-4" 
				placeholder="Enter Remark" <?php echo $disabled; ?>><?php if ($_POST) { echo $inputRemark; } else {echo $tampildata['Remark'];} ?></textarea></div>
			  </td>
            </tr>
			<tr>
              <td>Workflow Remark *</td>
              <td colspan="2"><div class="form-group"> <textarea cols="4"  name="inputWorkflowRemark"  class="form-control py-4" 
				placeholder="Workflow Remark" <?php echo $disabled; ?>><?php if ($_POST) { echo $inputWorkflowRemark; } ?></textarea></div>
			  </td>
            </tr>
          </table>
		...
		<button type="submit" name="Save" value="Update" onClick="return checkDraft(nprf)" class="btn btn-primary">Save</button>
...
$(document).ready(function(){
	   $('#add').on('show.bs.modal', function (e) {
		   ...
		   var LevelProcess = '<?php echo @$tampildataRevise['LevelProcess']; ?>';
		   var Index_No = '<?php echo @$tampildataRevise['Index_No']; ?>';
