...
<html lang="en">
    ...
    <div class="container-fluid"> 
      <h3 class="mt-4"><?php if ($button=="arm-add" || $button=="add-revise-arm" || $button<>"add-copy-arm") 
        ...
        <li class="breadcrumb-item active"><?php if ($button=="arm-add" || $button=="add-revise-arm" || $button<>"add-copy-arm") 
 	...
 	<div class="card mb-4">
		...
		from tb_packdev_add_resource a  LEFT JOIN tb_mpr b ON a.MPR_Code=b.Request_No
		...
		if (@$tampildata['Status_add_resource']<>"Draft" & @$tampildata['Status_add_resource']<>"" & 
			$button<>"add-revise-arm" & $button<>"revise-arm" & $button<>"add-copy-arm" )
	...
	if($_POST){
		...
		if($Save=="Save"){
			...
			if (mysqli_num_rows($TanyaReqWokflow) ==0 ) {  
				...
				$Tanya = mysqli_query($con,"SELECT Request_No FROM tb_packdev_add_resource WHERE Request_No = '$NomorReq' ");
		...
		elseif ($Save=="Revise"){
			...
			mysqli_query($con,"UPDATE tb_packdev_add_resource Set Status_add_resource='Revise' WHERE Request_No='$inputAutoRequestNo'");
		...
		elseif($Save=="Cancel"){ 
			include "arm-edit.php";
			mysqli_query($con,"UPDATE tb_packdev_add_resource SET Status_add_resource='Cancel' WHERE Request_No='$inputAutoRequestNo'");
		...
		elseif($Send=="NextandSave"){ 
			include "arm-autonumber.php";
			$Tanya = mysqli_query($con,"SELECT Request_No FROM tb_packdev_add_resource WHERE Request_No = '$inputAutoRequestNo' ");
			if (mysqli_num_rows($Tanya) !=0 ) {
				//Update Next
				include "arm-edit.php";
			}
			...
			//_________________________________________________________________________________________
			echo"<script>  window.location='../dist/index.php?button=arm-detail-add&id=$inputAutoRequestNo'; </script>";
		}
		elseif($Send=="Next"){ 
			//_________________________________________________________________________________________
			echo"<script>  window.location='../dist/index.php?button=arm-detail-add&id=$inputAutoRequestNo'; </script>";
		...
		?>
          <table width="100%" border="0" class="table table-striped" id="dataTables-example" >
            <tr>
			  ...
			  value="<?php if ($button=="arm-add" || $button=="add-copy-arm") {echo $NomorReq;} elseif ($button=="add-revise-arm") 
            ...
            <tr>
			  ...
			  value="<?php if (@$tampildata['Status_add_resource']=="Complete" & $button=="add-revise-arm") 
            ...
            <tr>
              ...
              <td width="30%"><div class="form-group d-flex align-items-center justify-content-between mt-0 mb-1">
				...
				placeholder="Master Product Request No"  readonly="readonly" 
				value="<?php if ($_POST) { echo $InputMPRCode; } else {echo @$tampildata['MPR_Code'];} ?>" 
				onChange="setFocus()" onFocus="setFocus()"  <?php echo $disabled; ?>>
		  ...
		  || @$tampildata['Status_add_resource']=="Revise" || $button=="add-revise-arm" || $button=="add-copy-arm"){ ?>
		  ...
		  value="<?php if ($button=="arm-add" || $button=="arm-revise"|| $button=="add-revise-arm" || $button=="add-copy-arm") 
		  {echo"Save";} else {echo"Update";}  ?>"
		  <?php if ($button=="arm-add" || $button=="arm-revise" || $button=="add-revise-arm") 
		  ...
		  (@$tampildata['Status_add_resource']=="Complete"  & $button=="add-revise-arm") ||
		  (@$tampildata['Status_add_resource']=="Complete"  & $button=="add-copy-arm") )  
		  {echo $disabled;} else {echo "disabled='disabled'";} ?>>Draft</button>
		  <?php ;} ?>
		  
		  <?php if (@$tampildata['Status_add_resource']=="Complete" & $button!="add-revise-arm")  {?>
		  <button type="submit" name="Save" value="Revise" onClick="return checkRevise()" 
		  class="btn btn-primary">Revise</button>
	      <?php ;} ?>
		  <?php if (@$tampildata['Status_add_resource']=="Complete" & $button!="add-revise-arm" || @$tampildata['Status_add_resource']=="Revise" & $button!="add-revise-arm")  {?>
		  <button type="submit" name="Save" value="Cancel" onClick="return checkCancel(arm)" 
		  class="btn btn-primary">Cancel Request</button>
	      <?php ;} ?>

		  <a class="btn btn-primary" href="../dist/index.php?button=arm" title="Back Format No Request">Back</a> 
		</form>
		</div>
      </div>
	 </div>
