...
<link href="../vendor/font-awesome/css/font-awesome.min.css" rel="stylesheet" type="text/css">
	...
	if($_POST){
		...
		if($Send=="Send"){ 
			if ($Confirm=="Approve"){
			/*	if (@$tampildataWFMPR['Index_No']=="1" || @$tampildataWFMPR['Index_No']=="4" 
					& @$tampildataWFMPR['LevelProcess']=="Step 1"){
					$Tanya = mysqli_query($con,"SELECT Request_No FROM tb_packdev_add_resource_detail
					 WHERE Request_No = '$inputAutoRequestNo' And (Vendor_Code='-' or Vendor_Code='') ");
					if (mysqli_num_rows($Tanya) !=0 ) {  
 						echo "<div class='form-group has-error'>
						<label class='control-label' for='inputError'>Error Approve : Vendor Code Can not be empty *</label></div>";
						return (false);	
					}else{
						return (true);	
					}
				}elseif (@$tampildataWFMPR['Index_No']=="1" || @$tampildataWFMPR['Index_No']=="2" || @$tampildataWFMPR['Index_No']=="5" 
					& @$tampildataWFMPR['LevelProcess']=="Step 4"){
					$Tanya = mysqli_query($con,"SELECT Request_No FROM tb_packdev_add_resource_detail
					 WHERE Request_No = '$inputAutoRequestNo' And (Resource_Code='-' or Resource_Code='') ");
					if (mysqli_num_rows($Tanya) !=0 ) {  
						echo "<div class='form-group has-error'>
						<label class='control-label' for='inputError'>Error Approve : Resource Code Can not be empty *</label></div>";
						return (false);	
					}else{
						return (true);	
					}
				...
				//UPDATE STATUS NPRF
				mysqli_query($con,"UPDATE tb_packdev_add_resource Set Status_add_resource='$StatusARM',
			...
			if ($Confirm=="Revise"){
					...
					WHERE Request_No='$tempFormatNoRequest'");
					 //UPDATE STATUS ARM
					mysqli_query($con,"UPDATE tb_packdev_add_resource 
          ...
          <table width="100%" border="0" class="table table-striped" id="dataTables-example" >
			...
			<tr>
              <td colspan="3"> 
				...
				FROM tb_packdev_add_resource_detail
...
/*	function checkSendApprovalPurchase(form){
 ...
 function get_PackagingARMDetail(){
  ...
  $.ajax({
   type: 'POST',
   url: "packaging-arm/arm-detail.php",
