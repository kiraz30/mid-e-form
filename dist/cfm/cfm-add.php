...
<html lang="en">
    ...
    <div class="container-fluid"> 
      <h3 class="mt-4"><?php if ($button=="add-cfm" || $button=="add-revise-cfm")
	  {echo "New Request CFM";} else  {echo "Edit Request CFM";} ?> </h3>
	    <ol class="breadcrumb mb-4">
        <li class="breadcrumb-item"><a href="../dist/index.php?button=dashboard">Dashboard</a></li>
		<li class="breadcrumb-item"><a href="../dist/index.php?button=cfm">Checklist Final Manuscript (CFM)</a></li>
        <li class="breadcrumb-item active"><?php if ($button=="add-cfm" || $button=="add-revise-cfm") 
		{echo "New Request CFM";} else  {echo "Edit Request CFM";} ?> </li>
      </ol>
 	<div class="card mb-4">
		...
		if ($button=="add-revise-cfm") {$query.=" And a.Status_Last_Document='1'";}
		else {$query =$query; }
		...
		if (@$tampildata['Status_CFM']<>"Draft" & @$tampildata['Status_CFM']<>"" & $button<>"add-revise-cfm")
		  {$disabled="disabled";} else{$disabled="";}
		if (@$tampildata['Status_CFM'] <> "Complete By MID" & $button=="add-revise-cfm")
	...
	if($_POST){
		...
		if($Save=="Save"){
			...
			if (mysqli_num_rows($Tanya) !=0 ) {  
				...
				include "cfm-autonumber.php";
				if ($button=="add-cfm") {
					include "cfm-save-new.php";
					include "cfm-save-file-new.php"; 
				}else {
					include "cfm-save-new-revisi.php";
					include "cfm-save-file-new-revisi.php"; 
				}
					
				include "cfm-save-workflow.php";
		...
		elseif ($Save=="Revise"){
			...
			mysqli_query($con,"UPDATE tb_cfm Set Status_CFM='Revise' WHERE Request_No='$inputAutoRequestNo'");
			...
			echo "<script type='text/javascript'>alert('$message');</script>";
			echo"<script>  window.location='../dist/index.php?button=revise-cfm&id=$inputAutoRequestNo'; </script>";
		...
		elseif($Send=="Send"){ 
			...
			if (mysqli_num_rows($Tanya) !=0 ) {
				...
				mysqli_query($con,"UPDATE tb_cfm SET Status_CFM='Sent' WHERE Request_No='$inputAutoRequestNo'");
				include "cfm-save-workflow.php";
			...
			NameApproval,OnBehalf,ReadInbox,Remark,WorkFlowMenu) values ('$inputAutoRequestNo','$createddate','$inputProductName',
			'$inputRequestType','W','".$tampildataNext['NameApproval']."',
			'".$tampildataNext['OnBehalf']."','0','$inputRemark','CFM')");
		...
		?>
          <table width="100%" border="0" class="table table-striped" id="dataTables-example" >
            ...
            <tr>
			  ...
			  placeholder="Auto Request No" readonly="readonly" 
			  value="<?php if (@$tampildata['Status_CFM']=="Complete" & $button=="add-revise-cfm") 
		 ...
		 onClick="popupwindow('../dist/page.php?form=cfm-comment&id=<?php echo $rowCFMFile['ID_No'];?>&pg=cfm-comment','CFM','400','1000');">History</button>	</td>
			...
			<?php if (@$tampildata['Status_CFM']<>"Complete") {?>	
			<tr>
		  ...
		  <?php if (@$tampildata['Status_CFM']!="Complete" || (@$tampildata['Status_CFM']=="Complete"  & $button=="add-revise-cfm") )  
		  {echo $disabled;} else {echo "disabled='disabled'";} ?>>Send Approval</button>
		  <?php if (@$tampildata['Status_CFM']<>"Cancel")  {?>
		  <button type="submit" name="Save" value="<?php if ($button=="add-cfm" || $button=="add-revise-cfm") 
		  {echo"Save";} else {echo"Update";}  ?>"
		  <?php if ($button=="add-cfm" || $button=="add-revise-cfm") 
		   {echo 'onclick="return checkDraft(cfm)"';} else {echo 'onclick="return checkEdit(cfm)"';} ?>  
		  class="btn btn-primary"
		  <?php if (@$tampildata['Status_CFM']!="Complete" || (@$tampildata['Status_CFM']=="Complete"  & $button=="add-revise-cfm") )  
		  {echo $disabled;} else {echo "disabled='disabled'";} ?>>Draft</button>
		  <?php ;} ?>
		  
		  <?php if (@$tampildata['Status_CFM']=="Complete" & $button!="add-revise-cfm")  {?>
