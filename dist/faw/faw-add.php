...
<html lang="en">
    ...
    <div class="container-fluid"> 
 	...
 	<div class="card mb-4">
		...
		if ($button=="add-revise-faw") {$query.=" And a.Status_Last_Document='1'";}
		else {$query =$query; }
		...
		if (@$tampildata['Status_FAW']<>"Draft" & @$tampildata['Status_FAW']<>"" & $button<>"add-revise-faw")
		  {$disabled="disabled";} else{$disabled="";}
		if (@$tampildata['Status_FAW'] <> "Complete" & $button=="add-revise-faw")
	...
	if($_POST){
		...
		if($Save=="Save"){
			...
			if (mysqli_num_rows($Tanya) !=0 ) {  
				...
				include "faw-autonumber.php";
				if ($button=="add-faw") {
					include "faw-save-new.php";
					include "faw-save-file-new.php"; 
				}else {
					include "faw-save-new-revisi.php";
					include "faw-save-file-new-revisi.php"; 
				}
					
				include "faw-save-workflow.php";
		...
		elseif ($Save=="Revise"){
			...
			mysqli_query($con,"UPDATE tb_faw Set Status_FAW='Revise' WHERE Request_No='$inputAutoRequestNo'");
			...
			echo "<script type='text/javascript'>alert('$message');</script>";
			echo"<script>  window.location='../dist/index.php?button=revise-faw&id=$inputAutoRequestNo'; </script>";
		...
		elseif($Send=="Send"){ 
			...
			if (mysqli_num_rows($Tanya) !=0 ) {
				...
				mysqli_query($con,"UPDATE tb_faw SET Status_FAW='Sent' WHERE Request_No='$inputAutoRequestNo'");
				include "faw-save-workflow.php";
			...
			'".$tampildataNext['OnBehalf']."','0','$inputRemark','FAW')");
		...
		?>
          <table width="100%" border="0" class="table table-striped" id="dataTables-example" >
            ...
            <tr>
              <td><span class="form-group">Checklist Final Manuscript Request No *</span></td>
			...
			<tr>
              ...
              <td>
				...
				maxlength="50" type="text" placeholder="Enter Material Name"  <?php echo $disabled; ?>
			...
			<tr>
              ...
              <td>
				...
				maxlength="50" type="text" placeholder="Enter Supplier Name"  <?php echo $disabled; ?>
		  ...
		  <?php if (@$tampildata['Status_FAW']!="Complete" || (@$tampildata['Status_FAW']=="Complete"  & $button=="add-revise-faw") )  
		  {echo $disabled;} else {echo "disabled='disabled'";} ?>>Send Approval</button>
		  <?php if (@$tampildata['Status_FAW']<>"Cancel")  {?>
		  <button type="submit" name="Save" value="<?php if ($button=="add-faw" || $button=="add-revise-faw") 
		  {echo"Save";} else {echo"Update";}  ?>"
		  <?php if ($button=="add-faw" || $button=="add-revise-faw") 
		   {echo 'onclick="return checkDraft(faw)"';} else {echo 'onclick="return checkEdit(faw)"';} ?>  
		  class="btn btn-primary"
		  <?php if (@$tampildata['Status_FAW']!="Complete" || (@$tampildata['Status_FAW']=="Complete"  & $button=="add-revise-faw") )  
	      ...
	      <?php ;} ?>
		  <?php if (@$tampildata['Status_FAW']=="Complete" & $button!="add-revise-faw" || @$tampildata['Status_FAW']=="Revise" & $button!="add-revise-faw")  {?>
