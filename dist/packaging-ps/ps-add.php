...
<html lang="en">
    ...
    <div class="container-fluid"> 
      <h3 class="mt-4"><?php if ($button=="ps-add" || $button=="add-revise-ps" || $button=="add-copy-ps") 
        ...
        <li class="breadcrumb-item active"><?php if ($button=="ps-add" || $button=="add-revise-ps" || $button=="add-copy-ps") 
 	...
 	<div class="card mb-4">
		...
		$query="SELECT a.ID_No,a.Request_No,a.Index_Document,a.Last_Request_No,a.Status_Last_Document,
		a.ARM_Code,a.MPR_Code,b.Project_Name,a.Finish_Good_Code,a.Finish_Good_Name,a.Keterangan_Produk,a.Warna_Jenis,
		a.Netto,a.Isi,a.DZ_CT,a.Market,a.Barcode,a.CreatedBy,DATE(a.CreatedDate) as CreatedDate,a.Status_Spec,a.Remark
		...
		if (@$tampildata['Status_Spec']<>"Draft" & @$tampildata['Status_Spec']<>"" & 
			$button<>"add-revise-ps" & $button<>"revise-ps" & $button<>"add-copy-ps" )
	...
	if($_POST){
		...
		$tempARM_ID_No_Detail	= @$_POST['tempARM_ID_No_Detail'];
		...
		$inputKeteranganProduk	= @$_POST['inputKeteranganProduk'];
		$inputWarnaJenis	 	= @$_POST['inputWarnaJenis'];
		...
		if($Save=="Save"){
			...
			WHERE UserDomain= '$username' And WorkFlowMenu= 'PS' And LevelApproval ='Requestor'");
			if (mysqli_num_rows($TanyaReqWokflow) ==0 ) {  
				...
				$Tanya = mysqli_query($con,"SELECT Request_No FROM tb_packdev_spec WHERE Request_No = '$NomorReq' ");
		...
		elseif ($Save=="Revise"){
			...
			if (mysqli_num_rows($Tanya) !=0 ) {
				...
				//membuat Query untuk menyimpan data
				if ($button=="add-revise-ps" || $button=="add-copy-ps"){
					include "ps-save-new.php";
					include "ps-detail-form-save-new-revise.php";
					include "ps-save-workflow.php";}
				else{
			...
			//_________________________________________________________________________________________
			echo"<script>  window.location='../dist/index.php?button=ps-detail-add&id=$NomorReq'; </script>";
		...
		?>
          <table width="100%" border="0" class="table table-striped" id="dataTables-example" >
            <tr>
			  ...
			  value="<?php if ($button=="ps-add" || $button=="add-copy-ps") {echo $NomorReq;} elseif ($button=="add-revise-ps") 
			   
			  {echo substr($tampildata['Request_No'],0,15)."-R".$rev;} else {echo @$tampildata['Request_No'];} ?>" />
              </span> </td>
            </tr>
            <tr>
			  ...
			  value="<?php if ($button=="add-revise-ps") 
			  {echo @$tampildata['Request_No'];} else if ($button=="add-copy-ps")
			  {echo "";} else   {echo @$tampildata['Last_Request_No'];}?>" />
              </span> </td>
            </tr>
			<tr>
              <td>
			  <span class="form-group">Add Resource Master Request No </span></td>
              <td width="30%"><div class="form-group d-flex align-items-center justify-content-between mt-0 mb-1">
			  <input name="tempARM_ID_No_Detail" id="tempARM_ID_No_Detail" type="hidden" value="<?php echo $tampildata['ARM_ID_No_Detail']; ?>">
			...
			<tr>
              <td width="20%"><span class="form-group">Keterangan Produk</span></td>
              <td colspan="2"><span class="form-group">
			  <input class="form-control py-4" name="inputKeteranganProduk" id="inputKeteranganProduk" 
				maxlength="150" type="text" placeholder="Input Keterangan Produk"   
				value="<?php if ($_POST) { echo $inputKeteranganProduk; } else {echo @$tampildata['Keterangan_Produk'];} ?>" 
				<?php echo $disabled; ?> />
              </span> </td>
            </tr> 
			<tr>
              <td width="20%"><span class="form-group">Warna / Jenis </span></td>
              <td ><span class="form-group">
			  <input class="form-control py-4" name="inputWarnaJenis" id="inputWarnaJenis" 
				maxlength="150" type="text" placeholder="Input Warna / Jenis"   
				value="<?php if ($_POST) { echo $inputWarnaJenis; } else {echo @$tampildata['Warna_Jenis'];} ?>" 
				<?php echo $disabled; ?> />
              </span> </td>
            </tr> 
			<tr>
		  ...
		  <?php if (@$button=="ps-add"){ ?>
			<button type="submit" name="Send" value="NextandNewSave" onClick="return checkSendApproval(ps)" 
			class="btn btn-primary">Save  and Continue
			<span   class="glyphicon glyphicon-menu-right" title="Save and Continue "></span></button>
		  <?php } elseif (@$tampildata['Status_Spec']=="" || @$tampildata['Status_Spec']=="Draft" 
		  || @$tampildata['Status_Spec']=="Revise" || $button=="add-revise-ps" || $button=="add-copy-ps"){ ?>
		  ...
		  value="<?php if ($button=="ps-add" || $button=="ps-revise"|| $button=="add-revise-ps" || $button=="add-copy-ps") 
		  {echo"Save";} else {echo"Update";}  ?>"
		  <?php if ($button=="ps-add" || $button=="ps-revise" || $button=="add-revise-ps" || $button=="add-copy-ps") 
		  ...
		  || (@$tampildata['Status_Spec']=="Complete"  & $button=="add-copy-ps"))  
		  {echo $disabled;} else {echo "disabled='disabled'";} ?>>Draft</button>
		  <?php ;} ?>
		  
		  <?php if (@$tampildata['Status_Spec']=="Complete" & $button!="add-revise-ps" & $button!="add-copy-ps")  {?>
	...
	/*else if(form.inputProjectName.value==""){
    	...
    	return (false);  		}*/
	else if(form.inputFinishGoodCode.value==""){
		alert("Finish Good Code Can not be empty!");
		form.inputFinishGoodCode.focus();
    	return (false);  		}
	else if(form.inputFinishGoodName.value==""){
		alert("Finish Good Name Can not be empty!");
		form.inputFinishGoodName.focus();
    	return (false);  		}
	else if(form.inputNetto.value==""){
		alert("Netto Can not be empty!");
		form.inputNetto.focus();
    	return (false);  		}
	else if (form.inputIsi.value == ""){
    	alert("Isi Can not be empty *");
    	form.inputIsi.focus();
    	return (false);  		}
	else if (form.inputDZCT.value == ""){
    	alert("DZ / CT Can not be empty *");
    	form.inputDZCT.focus();
    	return (false);  		}
	else if (form.inputMarket.value == ""){
    	alert("Market Can not be empty *");
    	form.inputMarket.focus();
    	return (false);  		}
