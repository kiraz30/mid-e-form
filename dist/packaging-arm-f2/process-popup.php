...
<html lang="en">
	...
	function changeparent(doc1,doc2){
        var input1=doc1;
		var input2=doc2;
		window.opener.document.getElementById('inputProcessCode').value=input1;
		window.opener.document.getElementById('inputProcessName').value=input2;
	    window.opener.close();
    }
</script>
	...
	<main> 
    <div class="container-fluid"><h3 class="mt-4">Search Process</h3>
	...
	&nbsp; <button type="submit" style="padding:4px 4px 4px 4px" 
         ...
         <div class="card-body"> 
          <div class="table-responsive">
			...
			// Query 
			$query = "SELECT code AS ProcessCode, descr AS ProcessDesc, createdate FROM tb_ms_proses msp";

			// Tambahkan filter jika ada
			if (@$Search == "") {
				$query = $query;
			} else {
				$query .= " WHERE msp.code LIKE '%$Search%' OR msp.descr LIKE '%$Search%'";
			}

			// Tambahkan urutan
			$query .= " ORDER BY msp.createdate DESC";
			...
			?>
				<tr> 
				<td><?php echo $no; ?></td>
				<td><?php echo htmlspecialchars($row['ProcessCode']); ?></td>
				<td><?php echo htmlspecialchars($row['ProcessDesc']); ?></td>
				<td align="center">
					<button type="button" style="padding:2px 4px 2px 2px" class="btn btn-outline btn-default"
					onClick="javascript:changeparent('<?php echo addslashes($row['ProcessCode']); ?>','<?php echo addslashes($row['ProcessDesc']); ?>');window.close();">
